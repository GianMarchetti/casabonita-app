...
export function TallerGallery({ taller }: Props) {
    ...
    <div className="flex flex-col-reverse">
      ...
      <div className="aspect-h-1 aspect-w-1 w-full">
        <Image
          as={NextImage}
          priority
          src={urlForImage(taller.images[selectedImage]).url()}
          alt={`Main ${taller.name} image`}
          width={600}
          height={750}
          placeholder="blur"
            blurDataURL={`data:image/svg+xml;base64,${toBase64(shimmer(600, 750))}`}
          className="h-full w-full z-0 border-2 border-gray-200 object-cover object-center shadow-sm dark:border-gray-800 sm:rounded-lg sm:pt-rem-2"
        />
      </div>
    </div>
  )
